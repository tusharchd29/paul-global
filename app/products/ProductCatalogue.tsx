...
export default function ProductCatalogue(){
 ...
 return <div className="catalogue">
  ...
  {active && <section id="product-list" className="selected-products"><div className="selected-head"><div><div className="eyebrow">02 · {problem?'Your problem':categoryMeta[cat]?.label}</div><h2 className="serif">{problem||categoryMeta[cat]?.label}<span className="slash"> / </span>Products</h2><p className="muted">{problem?'Products the catalogue identifies as a fit for this problem, across all relevant sectors.':categoryMeta[cat]?.intro}</p></div><button className="text-button" onClick={clearFilter}>← Start over</button></div>
  ...
  {!active&&<div className="catalogue-note"><span className="eyebrow">03 · Need technical information?</span><strong className="serif">Our technical team can help identify the right solution.</strong><Link className="btn btn-gold" href="/#technical">Explore technical capability ↗</Link></div>}
 </div>
}
