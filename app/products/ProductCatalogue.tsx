...
export default function ProductCatalogue(){
 ...
 const choose=(c:string)=>{setCat(c);setQ('');history.replaceState(null,'',`#${slugify(c)}`);setTimeout(()=>document.getElementById('product-list')?.scrollIntoView({behavior:'smooth',block:'start'}),30)};
 return <div className="catalogue">
  ...
  {cat && <section id="product-list" className="selected-products"><div className="selected-head"><div><div className="eyebrow">02 · {categoryMeta[cat]?.label}</div><h2 className="serif">{categoryMeta[cat]?.label}<span className="slash"> / </span>Products</h2><p className="muted">{categoryMeta[cat]?.intro}</p></div><button className="text-button" onClick={()=>{setCat('');history.replaceState(null,'',window.location.pathname)}}>← All categories</button></div>
